...
import {
  FiLayout, FiUserPlus, FiLogOut, FiLock,
  FiMenu, FiX, FiSave,
} from 'react-icons/fi';
import { GoSidebarExpand, GoSidebarCollapse } from 'react-icons/go';

const NAV_ITEMS = [
  { href: '/faculty/dashboard', icon: FiLayout,   label: 'Dashboard' },
  { href: '/faculty/register',  icon: FiUserPlus, label: 'Register'  },
];
...
export default function FacultyLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (user.role !== 'faculty') { router.push('/login'); return; }
  ...
  if (!initialized || !user || user.role !== 'faculty') return null;
  ...
  return (
    <div className="flex min-h-screen bg-slate-100 text-slate-900">
      ...
      <nav className="fixed bottom-0 inset-x-0 z-40 lg:hidden bg-blue-950 border-t border-white/10 flex safe-area-bottom">
        {NAV_ITEMS.map(({ href, icon: Icon, label }) => {
          const active = pathname.startsWith(href);
          return (
            <Link
          ...
          <div className="bg-white rounded-t-2xl sm:rounded-lg shadow-2xl w-full max-w-md p-5 sm:p-6 space-y-5" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium mt-0.5">Update your faculty account password</p>
              </div>
            ...
            <div className="flex gap-3 pt-1">
              <button onClick={handleChangePassword} disabled={changePwdSaving} className="flex items-center gap-2 bg-blue-950 text-white font-black px-5 py-2.5 rounded hover:bg-blue-700 transition shadow-sm active:scale-95 text-sm disabled:opacity-60">
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 text-center space-y-4">
            ...
            <div className="flex gap-3">
              <button onClick={stayLoggedIn} className="flex-1 bg-blue-600 text-white font-black py-2.5 rounded-lg hover:bg-blue-700 transition text-sm">
                Stay Logged In
              </button>
